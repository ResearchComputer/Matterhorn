...
class UI(Protocol):  # pragma: no cover
    def update(self, job: BaseJobStatus) -> None:
        """
        Displays the current state of Slurm job

        Args:
            job (JobStatus): The Slurm job
        """
...
class NullUI(UI):  # pragma: no cover
    ...
    def update(self, job: BaseJobStatus) -> None:  # pragma: no cover
        pass
...
class RichUI(UI):
    ...
    def _make_table(self, job: BaseJobStatus) -> Table:
